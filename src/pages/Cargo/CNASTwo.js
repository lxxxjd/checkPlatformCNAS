...
const CreateForm = Form.create()(props => {
  ...
  return (
    ...
    >
      ...
      <FormItem labelCol={{ span: 5 }} wrapperCol={{ span: 15 }} label="认可子领域编码">
        {form.getFieldDecorator('subDomainCode', {
          initialValue: modalInfo.subDomainCode,
          rules: [
            {
              required: true,
              message: "请输入CNAS认可子领域编码",
            },
          ],
        })(<Input placeholder="请输入CNAS认可子领域编码" />)}
      </FormItem>

      <FormItem labelCol={{ span: 5 }} wrapperCol={{ span: 15 }} label="认可子领域">
        {form.getFieldDecorator('subDomainName', {
          initialValue: modalInfo.subDomainName,
          rules: [
            {
              required: true,
              message: "请输入CNAS认可子领域",
            },
          ],
        })(<Input placeholder="请输入CNAS认可子领域" />)}
      </FormItem>
...
class CNASOne extends PureComponent {
...

    {
      ...
      render: (text, record) => (
        <Fragment>
          ...
          &nbsp;&nbsp;
          <a onClick={() => this.goCNASThree(text, record)}>编辑检查领域</a>
        </Fragment>
      ),
    },
  ];
  ...
  init =()=>{
    ...
    const code =  sessionStorage.getItem('cnasOne_domainCode');
    dispatch({
      type: 'cnas/getCNASLevelTwoList',
      payload:{code},
      callback: (response) => {
        if (response){
  ...
  goCNASThree =(text)=>{
    sessionStorage.setItem('cnasTwo_subDomainCode',text.subDomainCode);
    router.push({
      pathname:'CNASThree',
    });
  }
  ...
  handleEdit = (fields,modalInfo) => {
    const { dispatch } = this.props;
    let prams = modalInfo;
    prams.subDomainName =  fields.subDomainName;
    prams.subDomainCode =  fields.subDomainCode;
    const values = {
      ...prams,
    };
    dispatch({
      type: 'cnas/updateCNASLevelTwo',
      payload:values,
      callback: (response) => {
        if(response==="success"){
          ...
          <Col md={3} sm={20}>
            ...
            >
              {getFieldDecorator('kind', {
                initialValue:"subDomainName",
                rules: [{  message: '搜索类型' }],
              })(
                <Select placeholder="搜索类型" >
                  <Option value="subDomainName">分类名称</Option>
                  <Option value="subDomainCode">分类编码</Option>
